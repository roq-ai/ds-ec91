...
import { deleteRequestById } from 'apiSdk/requests';
...
function OrganizationViewPage() {
  ...
  const requestHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
              ...
              {hasAccess('request', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/requests/create?organization_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.request?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/requests/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>{record.type}</Td>
                            <Td>{record.status}</Td>
